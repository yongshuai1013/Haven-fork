...
- **GL acceleration — shipped virgl path, the GL-2.1 ceiling, and the venus deep dive (verified on a Pixel 8 Pro).** The cage GL broker shipped (v5.59.63): app GL in a Sway cage runs on Mali (`GL_RENDERER = virgl (Mali-G715)`) instead of llvmpipe. But virgl caps *desktop* GL at **2.1** — a hard ceiling, because Android's EGL exposes only **GLES** and virgl's GLES→desktop-GL *compatibility* profile is fixed at 2.1, so glxgears, older games, and most CAD viewers can't exceed it. An **ES2→ES3 host-context bump** (`vrend_winsys_egl.c`, rebuilt + device-tested) was a **verified dead-end** — it didn't raise the desktop-GL version (a higher GLES context only lifts *GLES-native* caps, which the software-Xwayland cage couldn't even measure: `glxinfo` → `X_GetImage BadMatch`, `eglinfo` hangs). The forward path is **venus** (pass the device's real Mali Vulkan into the guest) **+ Mesa zink** (GL-on-Vulkan → modern GL). Deep-dived over 2026-06-15; current state:
  ...
  - **Rendering hang — root-caused to a GPU-completion-fence gap in vtest+venus, NOT the GPU, memory, or AHB.** A minimal headless Vulkan probe (`venusfence.c`: an empty `vkQueueSubmit`+fence, then a per-memory-type `vkCmdFillBuffer`) isolated it: through venus the **empty submit completes**, but **real GPU work hangs for *every* memory type** — including a **device-local-only type that does NOT use the AHB import path** (so AHB is exonerated). The **same probe cross-compiled for Android passes against the real Mali ICD from `adb shell`** (and the virgl GLES path already runs in-process GPU jobs) → the **GPU and driver are fine**. Render-server instrumentation shows the host runs the work correctly — `vk->QueueSubmit` *and* a host-side `vk->WaitForFences` both return `VK_SUCCESS` (the GPU genuinely finishes) — yet the guest still spins `MESA-VIRTIO: stuck in fence wait`. **The completion is never signalled back to the guest.** Two reasons: (a) the guest's render submits arrive over vtest as `VCMD_SUBMIT_CMD2` batches with **`ring_idx=0`** and mostly `sync_count=0` (ring-kick pings, no GPU fence requested); the lone context fence that does arrive uses `ring_idx=0`, so `vkr_context_submit_fence` **retires it immediately on the CPU timeline, bypassing the GPU-wait sync queue**; and (b) venus's own ring sync (`vkr_transport.c` `write_extra` / `submit_virtqueue_seqno` / `wait_ring_seqno`) is **CPU-roundtrip ordering, never driven by GPU-fence completion**. A crude host-side blocking `WaitForFences` right after `QueueSubmit` **fixes standalone `vkWaitForFences`** (it serialises completion into the ring advance) but **not swapchain rendering** — es2gears and glmark2 still hang, because that path waits via semaphores/WSI, not a plain fence.
  - **What's reachable today.** With that workaround, `eglinfo` under zink-on-venus reports **`OpenGL core profile version: 3.2` (renderer `zink Vulkan 1.4 (Virtio-GPU Venus Mali-G715)`)** — past the virgl **2.1** ceiling. So the modern-GL *context* is reachable; only sustained app *rendering* is blocked by the fence gap. (zink is capped at 3.2 rather than 4.x by two missing Mali features `fillModeNonSolid`/`shaderClipDistance` — a separate, lesser issue.)
  - **The real fix — the next frontier.** Implement GPU-fence-driven completion signalling in the vtest+venus path: wait on the guest submit's host fence in a sync thread and write the completion seqno the guest polls — the equivalent of what the kernel virtio-gpu driver provides and vtest does not. This is upstream-virglrenderer-scale work, not a quick wiring change; it may be worth contributing upstream rather than carrying as a fork. **Status:** the AHB allocator + venus build patches + fence diagnostics are uncommitted working-tree changes (recoverable from the `wayland-android` submodule + the `/tmp/virgl-ahb` build dir + the `venusfence.c` probe); the device runs the clean shipped virgl build.
