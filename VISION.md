...
1. **Warm the Reticulum extremity** — give the only cold transport SFTP and a tunnelable port, so the mesh carries more than a shell (§1, growth diagnostic).
...
- **Extremities — Gateway/Reticulum: cold.** `ReticulumTransport` returns only an `RnshShellSession`. No SFTP-over-Reticulum, no tunnelable port, no file sync. Warming the extremity means giving Reticulum the same surface area SSH already has, not a parallel one — and it is now the *only* transport in the registry that doesn't carry its weight. This is the foregrounded next-iteration #1.
